...
;(function (window) {
  ...
  IQSetting.prototype.fnCall = function (key, value = this.getSettingKey(key).value) {
    if (_.isString(key)) {
  ...
  IQSetting.prototype.getStorageValue = function (key) {
    const checkKey = IQUtils.checkStorageArray(key, ['localStorage', 'sessionStorage', 'cookieStorage', 'none'])
  ...
  IQSetting.prototype.__styleUpdate__ = function (key, pair = { prop: '', value: 'value' }, cb) {
    const obj = this.getSettingKey(key)
    if (pair.value !== null) {
  ...
  IQSetting.prototype.__attributeUpdate__ = function (key, pair = { prop: 'color', value: 'value' }, cb) {
    const obj = this.getSettingKey(key)
    if (pair.value !== null) {
      obj.value = pair.value
      this.setSettingOption(key, pair.value)
    ...
    if (obj.target !== null) {
      ...
      if (!_.isObject(obj.value)) {
        _.forEach(obj.colors, (value, index) => {
          if (IQUtils.getElem(`[data-extra="${index.replace('--{{prefix}}', '')}"]`) !== null) {
            IQUtils.getElem(`[data-extra="${index.replace('--{{prefix}}', '')}"]`).value = value
          }
  ...
  this.IQSetting.prototype.resetColor = function (key) {
    const choices = defaults.setting.theme_color.choices.find((x) => x.name == 'custom')
  ...
  IQSetting.prototype.resetFontFamily = function () {
    ...
    document.documentElement.style.setProperty(`--${prefix}body-font-family`, bodyFamily)
    document.documentElement.style.setProperty(`--${prefix}heading-font-family`, headingFamily)
  ...
  IQSetting.prototype.theme_scheme_direction = function (value) {
    if (typeof value !== typeof null) {
      const __this = this
      this.__attributeUpdate__('theme_scheme_direction', { prop: 'dir', value: value }, function (key, val) {
