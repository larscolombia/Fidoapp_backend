...
function get_pet_center_id()
{
    ...
    return $pet_center ?  $pet_center->id : 1;
}
...
function SendPushNotification($data)
{
    ...
    $setting_data = \App\Models\Setting::where('type', 'mobile_config')->get();
    ...
    if ($setting_data->isNotEmpty()) {
        ...
        foreach ($setting_data as $setting) {
            if ($setting->name === 'employee_onesignal_app_id') {
                $employee_onesignal_app_id = $setting->val;
            } elseif ($setting->name === 'employee_onesignal_rest_api_key') {
                $employee_onesignal_rest_api_key = $setting->val;
            } elseif ($setting->name === 'onesignal_app_id') {
                $onesignal_app_id = $setting->val;
            } elseif ($setting->name === 'onesignal_rest_api_key') {
                $onesignal_rest_api_key = $setting->val;
            }
        }
    }
    ...
    if (isset($data['employee_id'])) {
        ...
        $employee = \App\Models\User::where('id', $data['employee_id'])->first();

        $employee_email = $employee->email;

        $fields = array(
            'app_id' =>  $employee_onesignal_app_id,
            // 'include_player_ids' => [$employee_player_id], // Wrap employee_id in an array
            'tags' => array(
                array('key' => 'email', 'relation' => '=', 'value' => $employee_email)
            ),
            'headings' => $heading,
            'contents' => $content,
        );
        $rest_api_key = $employee_onesignal_rest_api_key;
    } else {
        ...
        $fields = array(
            ...
            // 'include_player_ids' => [$user_player_id], // Assuming employee_ids is an array
            'tags' => array(
                array('key' => 'email', 'relation' => '=', 'value' => $user_email)
            ),
            'headings' => $heading,
            'contents' => $content,
        );
