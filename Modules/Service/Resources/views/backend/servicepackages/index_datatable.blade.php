                ...
                <x-slot name="toolbar">
                    ...
                    {{-- @can('add_' . $module_name)
                        <x-buttons.offcanvas target='#form-offcanvas'
                            title="{{ __('Create') }} {{ __($module_title) }}" class=" d-flex align-items-center gap-1">{{ __('messages.new') }}</x-buttons.offcanvas>
                    @endcan --}}
                    @hasPermission('create_service_package')
                        <button type="button" class="btn btn-primary" data-bs-toggle="tooltip"
                            data-crud-id="{{ 0 }}" title="{{ __('Create') }} {{ __($module_title) }}"><i
                                class="fas fa-plus-circle"></i>{{ __('Create') }} {{ __($module_title) }}</button>
                    @endhasPermission
                </x-slot>
            </x-backend.section-header>
        </div>
    ...
    <script type="text/javascript" defer>
        const columns = [{
            ...
            },
            {
                data: 'name',
                name: 'name',
                title: "{{ __('service_package.lbl_name') }}",
                width: '15%'
            },
            {
                data: 'category_id',
                name: 'category_id',
                title: "{{ __('service_package.lbl_category') }}",
                width: '15%'
            },
            {
                data: 'sub_category_id',
                name: 'sub_category_id',
                title: "{{ __('service_package.lbl_sub_category') }}",
                width: '15%'
            },
            {
                data: 'description',
                name: 'description',
                title: "{{ __('service_package.lbl_description') }}",
                width: '15%'
            },
            {
                data: 'price',
                name: 'price',
                title: "{{ __('service_package.lbl_price') }}",
            ...
            },
            {
                ...
                orderable: false,
                searchable: true,
                title: "{{ __('service_package.lbl_status') }}",
                width: '5%'
            },
        ]
