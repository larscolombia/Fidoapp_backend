            ...
            <x-backend.section-header>
                ...
                <x-slot name="toolbar">
                    @can('add_' . $module_name)
                        <x-buttons.create route='{{ route('backend.notification-templates.create') }}'
                    ...
                    @can('restore_' . $module_name)
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" id="module-dropdown-01">
                                <li>
                                    <a class="dropdown-item" href='{{ route('backend.notification-templates.trashed') }}'>
                        ...
                        <tbody>
                            @foreach ($$module_name as $module_name_singular)
                                <tr>
                                    ...
                                    <td>
                                        <a
                                            href="{{ url("app/$module_name", $module_name_singular->id) }}">{{ $module_name_singular->name }}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <a href='{!! route("backend.$module_name.edit", $module_name_singular) !!}' class='btn btn-sm btn-primary mt-1'
                                            data-bs-toggle="tooltip" title="Edit {{ __($module_title) }}"><i
                                                class="fas fa-wrench"></i></a>
                                        <a href='{!! route("backend.$module_name.show", $module_name_singular) !!}' class='btn btn-sm btn-success mt-1'
                                            data-bs-toggle="tooltip" title="Show {{ __($module_title) }}"><i
                                                class="fas fa-tv"></i></a>
                                    </td>
                                </tr>
                            @endforeach
