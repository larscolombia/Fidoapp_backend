...
<div class="card" >
        ...
        <div class="input-group flex-nowrap">
          ...
          <input type="text" class="form-control form-control-sm dt-search" placeholder="Search..." aria-label="Search" aria-describedby="addon-wrapping">
             ...
             @hasPermission('add_boarding_booking')
                   <x-buttons.offcanvas target='#form-offcanvas'  class="btn btn-primary d-flex align-items-center gap-1 d-block fw-normal"
                title="{{ __('Create') }} {{ __($create_title) }}">
  ...
  <x-backend.advance-filter>
    ...
    <form action="javascript:void(0)" class="datatable-filter">
      ...
      <div class="form-group">
        ...
        <select name="filter_employee_id" id="column_employee_id" name="column_employee_id" data-filter="select" class="select2 form-control" data-ajax--url="{{ route('backend.get_search_data', ['type' => 'boarder']) }}" data-ajax--cache="true">
        </select>
      </div>
  ...
  <div data-render="app">
        <booking-form create-title="{{ __('messages.create') }} {{ __($create_title) }} "
            edit-title="{{ __('messages.edit') }} {{ __('booking.lbl_boading') }} {{ __('booking.singular_title') }} ">
        </booking-form>
...
<script type="text/javascript">
  ...
  const columns = [{
    ...
    },
    {
      data: 'drop_date_time',
      name: 'drop_date_time',
      title: "{{ __('booking.dropoffdate_time') }}",
      searchable: true,
    },
    {
      data: 'pick_date_time',
      name: 'pick_date_time',
      title: "{{ __('booking.pickupdate_time') }}",
      searchable: true,
    },
