                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between flex-wrap">
                        <div class="d-flex gap-3 align-items-center">
                            <img src="{{ $booking->user->profile_image ?? default_user_avatar() }}" alt="avatar"
                                class="avatar avatar-70 rounded-pill">
                        ...
                        <div class="d-flex gap-5 align-items-center">
                            <div class="flex-column">
                                ...
                                <h6 class="m-0"> <img
                                        src="{{ optional($booking->pet)->pet_image ?? default_user_avatar() }}"
                                        alt="avatar" class="avatar avatar-30 rounded-pill">
                                    {{ $booking->pet ? optional($booking->pet)->name : '--' }}</h6>
                            </div>
                            <div class="flex-column">
                                <span class="mb-2">{{ __('pet.pet_breed') }}</span>
                                <h6 class="m-0">
                                    {{ $booking->pet ? optional(optional($booking->pet)->breed)->name : '--' }}</h6>
                            </div>
                            <div class="flex-column">
                                ...
                                @if ($booking->status === 'rejected')
                                    <h6 class="m-0">{{ 'Rechazado' }}</h6>
                                @endif
                                @if ($booking->status === 'confirmed')
                                    <h6 class="m-0">{{ 'Confirmado' }}</h6>
                                @endif
                                @if ($booking->status === 'completed')
                                    <h6 class="m-0">{{ 'Completado' }}</h6>
                                @endif
                    ...
                    <div class="row gap-5 mb-5">
                        ...
                        <div class="col-md-3">
                            @if ($booking->booking_type === 'boarding')
                                ...
                                @else
                                    <h6 class="m-0"> <img
                                            src="{{ $booking->employee->profile_image ?? default_user_avatar() }}"
                                            alt="avatar" class="avatar avatar-50 rounded-pill">
                                        {{ $booking->employee->first_name . ' ' . $booking->employee->last_name }}</h6>
                                @endif
                    ...
                    <div class="row gap-5 pt-5">
                        <div class="col-md-3">
                            ...
                            @if (isset($booking->payment->payment_status))
                                ...
                                @else
                                    <h6 class="m-0">{{ __('booking.paid') }}</h6>

                                    <span>{{ __('booking.lbl_payment_method') }}</span>
                                    @if (ucfirst($booking->payment->transaction_type) === 'Cash')
                                        <h6 class="m-0">Pagado con FidoCoins</h6>
                                    @else
                                        <h6 class="m-0">Pagado con {{ ucfirst($booking->payment->transaction_type) }}
                                        </h6>
                                    @endif
                                @endif
                            @else
                            ...
                            <div class="col-md-3">
                                ...
                                @if ($booking->veterinary->service->type == 'video-consultancy')
                                    @if ($booking->veterinary->start_video_link != '' && $booking->veterinary->join_video_link != '')
                                        <a href={{ $booking->veterinary->start_video_link }}
                                            class="badge booking-status bg-soft-success p-3" target="_blank">
                                            {{ __('booking.start_video') }} </a>
                                        <a href={{ $booking->veterinary->start_video_link }}
                                            class="badge booking-status bg-soft-danger p-3" target="_blank">
                                            {{ __('booking.join_video') }} </a>
                                    @endif
                                @endif
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center detail-header p-4 rounded">
                                <div class="detail-box bg-white rounded">
                                    <img src="{{ $booking->systemservice->feature_image ?? default_user_avatar() }}"
                                ...
                                <div class="ms-3 w-100 d-flex align-items-center justify-content-between flex-wrap">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <span><b>{{ ($booking->systemservice->name === 'training') ? 'entrenamiento' : 'veterinario' }}</b>
                                            ({{ $booking->systemservice->description }})</span>
                                    </div>
                                    <h5 class="mb-0 w-50 text-lg-end text-sm-start">
                                        {{ $booking->service_amount.$coin->symbol }}</h5>
                                </div>
                            </div>
                            <ul class="list-unstyled pt-4 mb-0">
                                ...
                                @if (is_array($tax))
                                    @foreach ($tax as $t)
                                        @if ($t['type'] == 'percentage')
                                            <li
                                                class="d-flex align-items-center justify-content-between pb-2 border-bottom border-light">
                                                <span>{{ $t['title'] }} ({{ $t['value'] }}%)</span>
                                                <?php
                                                $tax_amount = ($booking->service_amount * $t['value']) / 100;
                                                ?>
                                                <span class="text-primary">{{ $tax_amount.$coin->symbol }}</span>
                                            </li>
                                        @elseif($t['type'] == 'fixed')
                                            <li
                                                class="d-flex align-items-center justify-content-between pb-2 border-bottom border-light">
                                                <span>{{ $t['title'] }}</span>
                                                <span class="text-primary">{{ $t['value'].$coin->symbol }}</span>
                                            </li>
                                        @endif
                                    @endforeach
                                @endif
                                <li class="d-flex align-items-center justify-content-between pt-5">
                                    ...
                                    <h5 class="mb-0">{{ $booking->total_amount.$coin->symbol }}</h5>
                                </li>
                            </ul>
                        </div>
                    </div>
