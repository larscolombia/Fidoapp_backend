...
@section('content')
    <div class="card">
        ...
        <div data-render="app">
            <daycare-form create-title="{{ __('messages.create') }} {{ __($create_title) }}"
                edit-title="{{ __('messages.edit') }} {{ __('booking.lbl_day_care') }} {{ __('booking.singular_title') }} "></daycare-form>
        </div>
    </div>
@endsection
...
@push('after-scripts')
    ...
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            initDatatable({
                url: '{{ route('backend.daycare.index_data') }}',
                finalColumns,
                orderColumn: [
                    [9, 'desc']
                ],
