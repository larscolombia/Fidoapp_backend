...
<div class="card">
    ...
    <x-backend.section-header>
      ...
      <x-slot name="toolbar">
        ...
        <div class="input-group flex-nowrap">
          ...
          <x-buttons.offcanvas target='#form-offcanvas'  class="btn-primary  d-flex align-items-center gap-1"
                  title="{{ __('Create') }} {{ __($create_title) }}">{{ __('messages.new') }}
          </x-buttons.offcanvas>
          @endhasPermission
      </x-slot>
    </x-backend.section-header>
  ...
  <x-backend.advance-filter>
    ...
    <form action="javascript:void(0)" class="datatable-filter">
      ...
      <div class="form-group">
        ...
        <select name="filter_employee_id" id="column_employee_id" name="column_employee_id" data-filter="select" class="select2 form-control" data-ajax--url="{{ route('backend.get_search_data', ['type' => 'groomer']) }}" data-ajax--cache="true">
        </select>
      </div>
      <div class="form-group">
  ...
  <div data-render="app">
    <groomer-form  create-title="{{ __('messages.create') }} {{ __($create_title) }}"
            edit-title="{{ __('messages.edit') }} {{ __('booking.lbl_grooming') }} {{ __('booking.singular_title') }} "></groomer-form>
</div>
</div>
@endsection
...
<script type="text/javascript">
  ...
  document.addEventListener('DOMContentLoaded', (event) => {
    initDatatable({
      url: '{{ route("backend.grooming.index_data") }}',
      finalColumns,
      orderColumn: [[ 9, 'desc' ]],
      advanceFilter: () => {
        return {
