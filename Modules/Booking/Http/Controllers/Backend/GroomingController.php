...
class GroomingController extends Controller
{
    ...
    {
        // Page Title
        $this->module_title = 'menu.grooming_booking';
    ...
    public function index_list(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $updated_data[$key] = [
                ...
                'title' => $serviceName,
                'titleHTML' => view('booking::backend.grooming.calender.event', compact('serviceName', 'customerName'))->render(),
    ...
    {
        $module_action = 'List';
        $create_title = __('booking.grooming_booking');
        $type=$request->type;
        ...
        return view('booking::backend.grooming.index_datatable', compact('module_action', 'filter', 'booking_status', 'create_title','type'));
    }
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $employee = User::where('user_type', 'groomer')->get();
        ...
        return $datatable->eloquent($query)
            ...
            ->addColumn('action', function ($data) use ($module_name) {
                return view('booking::backend.grooming.datatable.action_column', compact('module_name', 'data'));
            })
            ->editColumn('status', function ($data) use ($booking_status, $booking_colors) {
                return view('booking::backend.grooming.datatable.select_column', compact('data', 'booking_status', 'booking_colors'));
            })
            ...
            ->editColumn('user_id', function ($data) {
                return view('booking::backend.grooming.datatable.user_id', compact('data'));
            })
            ->editColumn('employee_id', function ($data) use ($employee) {
                $serviceid = $data->grooming->service_id;
            ...
            ->editColumn('start_date_time', function ($data) {
                return customDate(optional($data->grooming)->date_time);
            })
            ->filterColumn('start_date_time', function ($query, $keyword) {
                if (!empty($keyword)) {
                    $query->whereHas('grooming', function ($q) use ($keyword) {
            ...
            ->editColumn('services', function ($data) {
                return optional($data->grooming)->service_name;
            },)
            ->orderColumn('services', function ($query, $order) {
                $query->orderBy(new Expression('(SELECT service_name FROM booking_grooming_mapping WHERE booking_id = bookings.id)'), $order);
            }, 1)
            ...
            //     $subquery = '(SELECT employee_id FROM booking_services WHERE booking_id = bookings.id LIMIT 1)';
