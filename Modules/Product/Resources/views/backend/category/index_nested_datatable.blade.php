...
@section('content')
                ...
                <x-slot name="toolbar">
                    <div>
                        <div class="datatable-filter">
                            <select name="column_category" id="column_category" class="select2 form-control"
                                data-filter="select" style="width: 100%">
                                <option value="">{{ __('product.all_categories') }}</option>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @if ($category->id == $parentID) selected @endif>
                    ...
                    @hasPermission('create_subcategories')
                        <button type="button" class="btn btn-primary" data-crud-id="{{ 0 }}"
                            title="{{ __('messages.create') }} {{ __($module_title) }}"><i
                                class="fas fa-plus-circle me-2"></i>{{ __('messages.create') }}
                            {{ __('category.singular_title') }}</button>
                    @endhasPermission
    ...
    <div data-render="app">
        <category-form-offcanvas default-image="{{ default_feature_image() }}"
            create-title="{{ __('messages.create') }}{{ __('category.new') }} {{ __('category.singular_title') }}"
            edit-title="{{ __('messages.edit') }}{{ __('category.sub_category') }}"
            create-nested-title="{{ __('messages.create') }}{{ __('new') }}  Sub {{ __($module_title) }}"
            edit-nested-title="{{ __('messages.edit') }} Sub {{ __($module_title) }}"
            :category-id="{{ isset($category->id) ? $category->id : 0 }}"></category-form-offcanvas>
    </div>
@endsection
    ...
    <script type="text/javascript">
        const columns = [{
            ...
            },
            {
                ...
                title: "{{ __('category.lbl_category') }}",
                width: '15%',
                searchable: false,
                orderable: false,
            },
            {
                data: 'created_at',
                name: 'created_at',
                title: "{{ __('category.lbl_created_at') }}",
                width: '15%'
            },
            {
                data: 'updated_at',
                name: 'updated_at',
                title: "{{ __('category.lbl_updated_at') }}",
                width: '15%'
            },
            {
                data: 'status',
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            initDatatable({
                url: '{{ route('backend.products-categories.index_nested_data') }}',
                finalColumns,
                orderColumn: [
                    [5, "desc"]
                ],
