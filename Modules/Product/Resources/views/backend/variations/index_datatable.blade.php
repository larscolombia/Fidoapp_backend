...
@section('content')
        ...
        <div class="card-body">
            <x-backend.section-header>
                <div class="d-flex flex-wrap gap-3">
                    <x-backend.quick-action url="{{ route('backend.variations.bulk_action') }}">
                        <div class="">
    ...
    <div data-render="app">
        <variations-form-offcanvas create-title="{{ __('messages.create') }} {{ __($module_title) }}"
            edit-title="{{ __('messages.edit') }} {{ __($module_title) }}"
            :customefield="{{ json_encode($customefield) }}">
        </variations-form-offcanvas>
    </div>
    <x-backend.advance-filter>
        <x-slot name="title">
            <h4>{{ __('variations.lbl_advanced_filter') }}</h4>
        </x-slot>
    ...
    <script type="text/javascript" defer>
        const columns = [{
            ...
            },
            {
                data: 'name',
                name: 'name',
                title: "{{ __('variations.lbl_name') }}",
                width: '20%',
            },
            {
                data: 'type',
                name: 'type',
                title: "{{ __('variations.lbl_type') }}",
                width: '20%',
            },
            {
                data: 'updated_at',
                name: 'updated_at',
                width: '20%',
                title: "{{ __('variations.updated_at') }}",
            ...
            },
            {
                ...
                orderable: true,
                searchable: true,
                title: "{{ __('variations.lbl_status') }}",
                width: '5%'
            },
        ...
        const actionColumn = [{
            ...
            orderable: false,
            searchable: false,
            title: "{{ __('variations.lbl_action') }}",
            width: '5%'
        }]
