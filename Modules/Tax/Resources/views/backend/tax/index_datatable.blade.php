...
<div class="card">
            ...
            <x-slot name="toolbar">
                <div>
                    <div class="datatable-filter">
                        <select name="column_status" id="column_status" class="select2 form-control p-10" data-filter="select" style="width: 100%">
                            <option value="">{{__('tax.all')}}</option>
                ...
                <div class="input-group flex-nowrap">
                  ...
                  @hasPermission('add_tax')
                      <button type="button" class="btn btn-primary d-flex align-items-center gap-1" data-crud-id="{{ 0 }}" title="{{ __('Create') }} {{ __($module_title) }}"><i class="icon-add-new"></i>{{ __('messages.new') }}</button>
                  @endhasPermission
              </x-slot>
        </x-backend.section-header>
    </div>
    ...
    <div data-render="app">
        <tax-form-offcanvas create-title="{{ __('messages.create') }} {{ __($module_title) }}"
